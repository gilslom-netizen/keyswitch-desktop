...
// ---------------------------------------------------------------------------
test('wordlists-extra: at least 5000 added words per language', () => {
  const extra = require('../src/engine/wordlists-extra');
  assert.ok(extra.EXTRA_EN.length >= 5000, `EXTRA_EN has only ${extra.EXTRA_EN.length}`);
  assert.ok(extra.EXTRA_HE.length >= 5000, `EXTRA_HE has only ${extra.EXTRA_HE.length}`);
});
