...
// Distinct confirmation sound for the manual shortcut (a warm ascending chime,
// generated by tools/generate-notify-manual.js). Its own data: URI is a WAV so
// the renderer-Audio fallback can play it without an mp3 decode.
let manualSoundDataUri = null;
let manualWavBuffer = null;
function loadNotifySound() {
  ...
  try {
    manualWavBuffer = fs.readFileSync(path.join(__dirname, '..', 'assets', 'notify-manual.wav'));
    manualSoundDataUri = 'data:audio/wav;base64,' + manualWavBuffer.toString('base64');
  } catch (e) {
    manualWavBuffer = null;
    manualSoundDataUri = null;
  }
}
...
async function manualConvert() {
  ...
  try {
    ...
    if (settings.get('manualSound') !== false) playNotifySound('manual');
...
// Play a notification sound. Fastest path first: the WAV played natively from
// this process (starts within milliseconds). Fallbacks: the toast renderer's
// Audio pipeline, then the system beep. `which` selects the auto-correction
// sound (default) or the distinct manual-shortcut chime.
function playNotifySound(which) {
  const wav = which === 'manual' ? manualWavBuffer : notifyWavBuffer;
  const dataUri = which === 'manual' ? manualSoundDataUri : notifySoundDataUri;
  try {
    if (wav && native.playWav && native.playWav(wav)) return;
    if (dataUri && toastWin && !toastWin.isDestroyed()) {
      sendToToast('sound:play', dataUri);
    } else {
      native.beep();
    }
  } catch (e) {
